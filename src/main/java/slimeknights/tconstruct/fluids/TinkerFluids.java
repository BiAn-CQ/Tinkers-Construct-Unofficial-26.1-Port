...
public final class TinkerFluids extends TinkerModule {
  public TinkerFluids() {
    ...
    NeoForge.EVENT_BUS.addListener(this::registerCauldronInteractions);
  }

  @SubscribeEvent
  void registerCapabilities(RegisterCapabilitiesEvent event) {
    for (Item item : BuiltInRegistries.ITEM) {
      if (item instanceof PotionBucketItem bucket) {
        event.registerItem(Capabilities.Fluid.ITEM, (stack, context) -> bucket.getFluidHandler(context), item);
      } else if (item instanceof MagmaBottleItem bottle) {
        event.registerItem(Capabilities.Fluid.ITEM, (stack, context) -> bottle.getFluidHandler(context), item);
      } else if (item instanceof FluidContainerFoodItem food) {
        event.registerItem(Capabilities.Fluid.ITEM, (stack, context) -> food.getFluidHandler(context), item);
      }
    }
    event.registerItem(Capabilities.Fluid.ITEM,
      (stack, context) -> new ConstantFluidContainerWrapper(new FluidStack(powderedSnow.get(), FluidType.BUCKET_VOLUME), context, ItemResource.of(Items.BUCKET)),
  ...
  public static final ItemObject<Item> venomBottle = ITEMS.register("venom_bottle", () -> new FluidContainerFoodItem(itemProperties().food(TinkerFood.VENOM_BOTTLE.food(), TinkerFood.VENOM_BOTTLE.consumable()).stacksTo(16).craftRemainder(Items.GLASS_BOTTLE), () -> new FluidStack(venom.get(), FluidValues.BOTTLE)));
  ...
  public static final FlowingFluidObject<SlimeFluid> earthSlime = FLUIDS.registerSlime("earth_slime").type(() -> new SlimeFluidType(slime("earth_slime").temperature(350))).bucket().block(createEffect(MapColor.GRASS, 0, () -> new MobEffectInstance(TinkerEffects.bouncy, 5*20))).commonTag("slime").flowing(SlimeFluid.Source::new, SlimeFluid.Flowing::new);
  public static final FlowingFluidObject<SlimeFluid> skySlime   = FLUIDS.registerSlime("sky_slime"  ).type(() -> new SlimeFluidType(slime("sky_slime"  ).temperature(310))).bucket().block(createEffect(MapColor.DIAMOND, 0, () -> new MobEffectInstance(TinkerEffects.ricochet, 5*20))).flowing(SlimeFluid.Source::new, SlimeFluid.Flowing::new);
  public static final FlowingFluidObject<SlimeFluid> enderSlime = FLUIDS.registerSlime("ender_slime").type(() -> new SlimeFluidType(slime("ender_slime").temperature(370))).bucket().block(createEffect(MapColor.COLOR_PURPLE, 0, () -> new MobEffectInstance(TinkerEffects.enderference, 5 * 20))).flowing(SlimeFluid.Source::new, SlimeFluid.Flowing::new);
  ...
  public static final FlowingFluidObject<InvertedFluid> ichor   = FLUIDS.registerSlime("ichor").invertedType(() -> new SlimeFluidType.Inverted(slime("ichor").temperature(1000).density(-1600))).bucket().block(MapColor.COLOR_ORANGE, 0).invertedFlowing();
  ...
  public static final EnumObject<SlimeType, Item> slimeBottle = ITEMS.registerEnum(SlimeType.values(), "slime_bottle", type -> new FluidContainerFoodItem(
      itemProperties().food(TinkerFood.getBottle(type).food(), TinkerFood.getBottle(type).consumable()).stacksTo(16).craftRemainder(Items.GLASS_BOTTLE), () -> new FluidStack(slime.get(type), FluidValues.BOTTLE)));
  public static final ItemObject<Item> magmaBottle = ITEMS.register("magma_bottle", () -> new MagmaBottleItem(itemProperties().stacksTo(16).craftRemainder(Items.GLASS_BOTTLE), 15));

  // foods
  public static FlowingFluidObject<BaseFlowingFluid> honey        = FLUIDS.registerSlime("honey").type(() -> new SlimeFluidType(slime("honey").temperature(301))).bucket().block(createEffect(MapColor.COLOR_ORANGE, 0, () -> new MobEffectInstance(MobEffects.SLOWNESS, 5*20))).commonTag().flowing();
  ...
  public static final ItemObject<Item> meatSoupBowl = ITEMS.register("meat_soup", () -> new ContainerFoodItem(itemProperties().food(TinkerFood.MEAT_SOUP.food(), TinkerFood.MEAT_SOUP.consumable()).stacksTo(1).craftRemainder(Items.BOWL)));

  // potion
  public static final FluidObject<UnplaceableFluid> potion = FLUIDS.register("potion").type(() -> new PotionFluidType(cool().descriptionId("item.minecraft.potion.effect.empty").density(1100).viscosity(1100).temperature(315).sound(SoundActions.BUCKET_FILL, SoundEvents.BOTTLE_FILL).sound(SoundActions.BUCKET_EMPTY, SoundEvents.BOTTLE_EMPTY))).bucket(fluid -> new PotionBucketItem(fluid, ItemDeferredRegister.bindItemProperties(RegistrationHelper.BUCKET_PROPS))).commonTag().unplacable();
  public static final ItemObject<Item> splashBottle = ITEMS.register("splash_bottle", () -> new BottleItem(Items.SPLASH_POTION, itemProperties(ITEM_PROPS)));
  public static final ItemObject<Item> lingeringBottle = ITEMS.register("lingering_bottle", () -> new BottleItem(Items.LINGERING_POTION, itemProperties(ITEM_PROPS)));
  ...
  public static final FlowingFluidObject<InvertedFluid>     moltenCinderslime = FLUIDS.registerMetal("molten_cinderslime").invertedType(() -> new SlimeFluidType.Inverted(hot("molten_cinderslime").temperature(1350).lightLevel(10).density(-2000))).burningBlock(MapColor.COLOR_RED, 10, 10, 7f).bucket().invertedFlowing();
  ...
  public static final DeferredHolder<EntityDataSerializer<?>, ? extends EntityDataSerializer<?>> FLUID_DATA_SERIALIZER_REGISTRY = DATA_SERIALIZERS.register("fluid", () -> FLUID_DATA_SERIALIZER);
  ...
  void registerBrewingRecipes(RegisterBrewingRecipesEvent event) {
    ...
    builder.addRecipe(new BottleBrewingRecipe(TinkerIngredients.of(Items.GLASS_BOTTLE), TinkerIngredients.of(Items.GUNPOWDER), new ItemStack(splashBottle)));
    builder.addRecipe(new BottleBrewingRecipe(TinkerIngredients.of(MantleTags.Items.SPLASH_BOTTLE), TinkerIngredients.of(Items.DRAGON_BREATH), new ItemStack(lingeringBottle)));
    for (SlimeType slime : SlimeType.values()) {
      builder.addRecipe(new BrewingRecipe(TinkerIngredients.of(Items.GLASS_BOTTLE), TinkerIngredients.of(TinkerWorld.congealedSlime.get(slime)), new ItemStack(TinkerFluids.slimeBottle.get(slime))));
    }
    builder.addRecipe(new BrewingRecipe(TinkerIngredients.of(Items.GLASS_BOTTLE), TinkerIngredients.of(Blocks.MAGMA_BLOCK), new ItemStack(TinkerFluids.magmaBottle)));
  }
  ...
  /** Item component prototypes are bound before the game server starts on NeoForge 26.1. */
  private void registerCauldronInteractions(ServerStartingEvent event) {
    CauldronInteractions.WATER.put(Items.SPLASH_POTION, new EmptyBottleIntoWater(splashBottle, CauldronInteractions.WATER.get(new ItemStack(Items.SPLASH_POTION))));
    CauldronInteractions.WATER.put(Items.LINGERING_POTION, new EmptyBottleIntoWater(lingeringBottle, CauldronInteractions.WATER.get(new ItemStack(Items.LINGERING_POTION))));
    CauldronInteractions.EMPTY.put(Items.SPLASH_POTION, new EmptyBottleIntoEmpty(splashBottle, CauldronInteractions.EMPTY.get(new ItemStack(Items.SPLASH_POTION))));
    CauldronInteractions.EMPTY.put(Items.LINGERING_POTION, new EmptyBottleIntoEmpty(lingeringBottle, CauldronInteractions.EMPTY.get(new ItemStack(Items.LINGERING_POTION))));
  }
