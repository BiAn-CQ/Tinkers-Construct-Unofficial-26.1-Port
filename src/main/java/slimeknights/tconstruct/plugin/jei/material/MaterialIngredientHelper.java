...
public class MaterialIngredientHelper implements IIngredientHelper<MaterialVariant> {
  ...
  public ItemStack getCheatItemStack(MaterialVariant material) {
    ...
    if (recipe != null) {
      ItemStack[] items = slimeknights.tconstruct.library.recipe.TinkerIngredients.getItems(recipe.getIngredient());
      if (items.length > 0) {
        return items[0].copy();
      }
    }
  ...
  @Override
  public Identifier getIdentifier(MaterialVariant material) {
    return material.getId().location();
  }
  ...
  @Override
  public Stream<Identifier> getTagStream(MaterialVariant material) {
