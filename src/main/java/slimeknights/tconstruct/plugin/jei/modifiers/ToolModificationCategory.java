...
public class ToolModificationCategory extends AbstractTinkerStationCategory<IDisplayToolModification> {
  ...
  public void createRecipeExtras(IRecipeExtrasBuilder builder, IDisplayToolModification recipe, IFocusGroup focuses) {
    ...
    builder.addText(recipe.getTitle(), 124, 10).setTextAlignment(HorizontalAlignment.CENTER).setPosition(3, 3);
  }

  @Override
  public void getTooltip(mezz.jei.api.gui.builder.ITooltipBuilder tooltip, IDisplayToolModification recipe,
                         mezz.jei.api.gui.ingredient.IRecipeSlotsView slots, double x, double y) {
    super.getTooltip(tooltip, recipe, slots, x, y);
    if (x >= 3 && x < 127 && y >= 3 && y < 13) {
      tooltip.add(recipe.getTooltip());
    }
  }
}
