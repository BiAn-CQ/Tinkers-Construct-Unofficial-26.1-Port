...
public class ToolTinkeringExtension implements ICraftingCategoryExtension<AbstractCraftingTinkeringRecipe> {
  public static final ToolTinkeringExtension INSTANCE = new ToolTinkeringExtension();
  @Nullable
  ...
  @Override
  public List<net.minecraft.world.item.crafting.display.SlotDisplay> getIngredients(net.minecraft.world.item.crafting.RecipeHolder<AbstractCraftingTinkeringRecipe> holder) {
    AbstractCraftingTinkeringRecipe recipe = holder.value();
    List<net.minecraft.world.item.crafting.display.SlotDisplay> inputs = new ArrayList<>();
    inputs.add(stackDisplay(recipe.getToolWithoutModifier()));
    for (int i = 0; i < recipe.getInputCount(); i++) {
      inputs.add(stackDisplay(recipe.getDisplayItems(i)));
    }
    return inputs;
  }

  private static net.minecraft.world.item.crafting.display.SlotDisplay stackDisplay(List<ItemStack> stacks) {
    return new net.minecraft.world.item.crafting.display.SlotDisplay.Composite(stacks.stream().filter(stack -> !stack.isEmpty())
      .<net.minecraft.world.item.crafting.display.SlotDisplay>map(stack -> new net.minecraft.world.item.crafting.display.SlotDisplay.ItemStackSlotDisplay(net.minecraft.world.item.ItemStackTemplate.fromNonEmptyStack(stack))).toList());
  }

  @Override
  public void setRecipe(net.minecraft.world.item.crafting.RecipeHolder<AbstractCraftingTinkeringRecipe> holder, IRecipeLayoutBuilder builder, ICraftingGridHelper craftingGridHelper, IFocusGroup focuses) {
    AbstractCraftingTinkeringRecipe recipe = holder.value();
    ...
    // make slots
    List<IRecipeSlotBuilder> inputSlots = craftingGridHelper.createAndSetInputs(builder, mezz.jei.api.constants.VanillaTypes.ITEM_STACK, inputs, size, size);
    ...
    if (toolSize > 1) {
      ...
      if (linkToOutput.length > 0) {
        ...
        IRecipeSlotBuilder[] linked = Stream.concat(
          ...
          Arrays.stream(linkToOutput).filter(i -> i < inputCount && inputs.get(i+1).size() == toolSize).mapToObj(i -> inputSlots.get(getCraftingIndex(i+1, size, size)))
  ...
  @Override
  public void onDisplayedIngredientsUpdate(net.minecraft.world.item.crafting.RecipeHolder<AbstractCraftingTinkeringRecipe> holder, List<IRecipeSlotDrawable> recipeSlots, IFocusGroup focuses) {
    AbstractCraftingTinkeringRecipe recipe = holder.value();
    // handle dynamic hook
  ...
  @Override
  public void createRecipeExtras(net.minecraft.world.item.crafting.RecipeHolder<AbstractCraftingTinkeringRecipe> holder, IRecipeExtrasBuilder builder, ICraftingGridHelper craftingGridHelper, IFocusGroup focuses) {
    AbstractCraftingTinkeringRecipe recipe = holder.value();
    // add title and tooltip via an icon
    if (information != null) {
      builder.addDrawable(information, 56, 0);
      builder.addWidget(new slimeknights.tconstruct.plugin.jei.util.TooltipArea(56, 0, information.getWidth(), information.getHeight(), recipe.getInformation()));
    }
