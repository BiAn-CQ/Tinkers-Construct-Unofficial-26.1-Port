...
public class ToolBuildingCategory implements TinkersRecipeCategory<ToolBuildingRecipe> {
  ...
  public void draw(ToolBuildingRecipe recipe, IRecipeSlotsView recipeSlotsView, GuiGraphicsExtractor graphics, double mouseX, double mouseY) {
    ...
    // The old implementation used the same transform; losing it during the 26.1 GUI
    // migration makes the result look empty or like an untextured item in JEI.
    ...
    var renderPose = graphics.pose();
    renderPose.pushMatrix();
    renderPose.translate(5.0f, 6.5f);
