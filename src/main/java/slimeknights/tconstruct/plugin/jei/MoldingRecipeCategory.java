...
public class MoldingRecipeCategory implements TinkersRecipeCategory<MoldingRecipe> {
  ...
  public void draw(MoldingRecipe recipe, IRecipeSlotsView slots, GuiGraphicsExtractor graphics, double mouseX, double mouseY) {
    ...
    if (recipe.getPattern() != TinkerIngredients.EMPTY) {
  ...
  @Override
  public void getTooltip(mezz.jei.api.gui.builder.ITooltipBuilder tooltip, MoldingRecipe recipe, IRecipeSlotsView slots, double mouseX, double mouseY) {
    if (recipe.isPatternConsumed() && recipe.getPattern() != TinkerIngredients.EMPTY && GuiUtil.isHovered((int)mouseX, (int)mouseY, 50, 7, 18, 18)) {
  ...
  public void setRecipe(IRecipeLayoutBuilder builder, MoldingRecipe recipe, IFocusGroup focuses) {
    ...
    if (pattern != TinkerIngredients.EMPTY) {
