...
public class SimpleItemRecipeManager<T extends FilteredItemRecipe> implements ISimpleRecipeManagerPlugin<T> {
  ...
  public static <T extends slimeknights.tconstruct.library.recipe.tinkerstation.AbstractCraftingTinkeringRecipe>
      ISimpleRecipeManagerPlugin<net.minecraft.world.item.crafting.RecipeHolder<CraftingRecipe>> createCrafting(IIngredientManager ingredientManager, List<T> recipes) {
    SimpleItemRecipeManager<T> delegate = new SimpleItemRecipeManager<>(ingredientManager, recipes);
    return new ISimpleRecipeManagerPlugin<>() {
      private List<net.minecraft.world.item.crafting.RecipeHolder<CraftingRecipe>> wrap(List<T> values) {
        return values.stream().map(recipe -> new net.minecraft.world.item.crafting.RecipeHolder<CraftingRecipe>(
          net.minecraft.resources.ResourceKey.create(net.minecraft.core.registries.Registries.RECIPE, recipe.getId()), recipe)).toList();
      }
      public boolean isHandledInput(ITypedIngredient<?> input) { return delegate.isHandledInput(input); }
      public boolean isHandledOutput(ITypedIngredient<?> output) { return delegate.isHandledOutput(output); }
      public List<net.minecraft.world.item.crafting.RecipeHolder<CraftingRecipe>> getRecipesForInput(ITypedIngredient<?> input) { return wrap(delegate.getRecipesForInput(input)); }
      public List<net.minecraft.world.item.crafting.RecipeHolder<CraftingRecipe>> getRecipesForOutput(ITypedIngredient<?> output) { return wrap(delegate.getRecipesForOutput(output)); }
      public List<net.minecraft.world.item.crafting.RecipeHolder<CraftingRecipe>> getAllRecipes() { return wrap(delegate.getAllRecipes()); }
    };
  }
}
