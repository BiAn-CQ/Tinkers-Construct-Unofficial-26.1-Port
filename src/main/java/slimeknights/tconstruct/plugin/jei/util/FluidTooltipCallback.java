...
public interface FluidTooltipCallback extends IRecipeSlotRichTooltipCallback {
  ...
  default void onRichTooltip(IRecipeSlotView recipeSlotView, ITooltipBuilder builder) {
    ...
    for (Either<FormattedText, TooltipComponent> line : builder.getLines()) {
      Optional<FormattedText> text = line.left();
      if (text.isEmpty()) {
        onlyText = false;
        break;
      }
      FormattedText formatted = text.get();
      tooltip.add(formatted instanceof Component component ? component : Component.literal(formatted.getString()));
    }
    ...
    if (onlyText) {
      tooltip.addAll(appended);
      builder.clear();
      builder.addAll(tooltip);
    } else {
      builder.addAll(appended);
    }
  }
