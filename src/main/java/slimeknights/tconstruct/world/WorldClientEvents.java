...
public class WorldClientEvents extends ClientEventBase {
  ...
  static void clientSetup(FMLClientSetupEvent event) {
    // skull textures
    event.enqueueWork(() -> {
      // The 26.1 native minecraft:head item model resolves its kind through
      // SkullBlock.Type.TYPES.  NeoForge's skull model event accepts custom
      // Type implementations, but does not add them to this codec map.
      for (TinkerHeadType type : TinkerHeadType.values()) {
        SkullBlock.Type.TYPES.putIfAbsent(type.getSerializedName(), type);
      }
  ...
  /** Registers a skull texture for both block and native item head rendering. */
  ...
  /** Registers a piglin skull texture for both block and native item head rendering. */
