...
public enum TinkerHeadType implements Type, StringRepresentable {
  ...
  /*
   * Minecraft 26.1 resolves the kind used by the native minecraft:head item
   * model through SkullBlock.Type.CODEC. NeoForge's CreateSkullModels event
   * registers the renderer, but deliberately does not add custom types to
   * the codec lookup table. Register these as soon as the enum is initialized
   * so item model parsing during the first resource reload can resolve them.
   */
  static {
    for (TinkerHeadType type : values()) {
      Type.TYPES.putIfAbsent(type.getSerializedName(), type);
    }
  }
