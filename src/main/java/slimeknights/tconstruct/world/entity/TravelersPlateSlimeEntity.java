...
public abstract class TravelersPlateSlimeEntity extends ArmoredSlimeEntity {
  ...
  protected void populateDefaultEquipmentSlots(RandomSource random, DifficultyInstance difficulty) {
    ...
    if (this.getRandom().nextFloat() < 0.15F * multiplier) {
      ...
      if (this.getRandom().nextFloat() < 0.35f * multiplier) {
      ...
      for (int i = 0; i < max; i++) {
        if (this.getRandom().nextFloat() > 0.5f * multiplier) {
          break;
        }
        persistentData.addSlots(SlotType.DEFENSE, -1);
        modifiers = modifiers.withModifier(randomModifier(this.getRandom(), TinkerTags.Modifiers.SLIME_DEFENSE), 1);
      }
      // chance of diamond or emerald
      if (tool.getFreeSlots(SlotType.UPGRADE) > 0 && this.getRandom().nextFloat() < 0.5f * multiplier) {
        persistentData.addSlots(SlotType.UPGRADE, -1);
        modifiers = modifiers.withModifier(randomModifier(this.getRandom(), TinkerTags.Modifiers.GEMS), 1);
      }
