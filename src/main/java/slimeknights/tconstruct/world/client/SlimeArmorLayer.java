...
public class SlimeArmorLayer<T extends Slime, M extends HierarchicalModel<T>, A extends HumanoidModel<T>> extends RenderLayer<T,M> {
  ...
  private static void renderModel(PoseStack matrices, MultiBufferSource buffer, int packedLight, boolean enchanted, Model model, float red, float green, float blue, Identifier texture) {
    VertexConsumer vertexconsumer = ItemRenderer.getArmorFoilBuffer(buffer, RenderType.armorCutoutNoCull(texture), enchanted);
    AbstractArmorModel.renderColored(model, matrices, vertexconsumer, packedLight, OverlayTexture.NO_OVERLAY, -1, red, green, blue, 1.0F);
  }
  ...
  public static Identifier getArmorResource(Entity entity, ItemStack stack, ArmorItem armor, String type) {
    return slimeknights.tconstruct.library.tools.helper.ArmorUtil.getDummyArmorTexture(EquipmentSlot.HEAD);
  }
}
