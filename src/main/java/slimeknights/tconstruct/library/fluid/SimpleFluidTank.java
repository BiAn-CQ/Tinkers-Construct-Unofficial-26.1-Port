 ...
 */
public interface SimpleFluidTank extends IFluidTank, IndexedFluidHandler {
  @Override
  ...
  default int fill(int tank, FluidStack resource, FluidAction action) {
    return tank == 0 ? fill(resource, action) : 0;
  }
  ...
  @Nonnull
  @Override
  default FluidStack drain(int tank, FluidStack resource, FluidAction action) {
    return tank == 0 ? drain(resource, action) : FluidStack.EMPTY;
  }
