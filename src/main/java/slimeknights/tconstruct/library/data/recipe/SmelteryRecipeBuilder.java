...
public class SmelteryRecipeBuilder {
  ...
  @CheckReturnValue
  public static SmelteryRecipeBuilder fluid(RecipeOutput consumer, Identifier name, FluidObject<?> fluid) {
  ...
  @CheckReturnValue
  public static SmelteryRecipeBuilder fluid(RecipeOutput consumer, Identifier name, @Nullable Fluid fluid, @Nullable TagKey<Fluid> fluidTag) {
  ...
  @CheckReturnValue
  public static SmelteryRecipeBuilder fluid(RecipeOutput consumer, Identifier name, Fluid fluid) {
    return fluid(consumer, name, fluid, null);
  }

  /** Creates a builder for the given fluid tags, used as input and output */
  @CheckReturnValue
  public static SmelteryRecipeBuilder fluid(RecipeOutput consumer, Identifier name, TagKey<Fluid> fluidTag) {
  ...
  private FluidIngredient ingredient(int amount) {
    ...
    if (fluidTag != null) {
      return FluidTinkerIngredients.of(fluidTag, amount);
    }
    assert fluid != null;
    return FluidTinkerIngredients.of(fluid, amount);
  }
  ...
  private void minecraftArmorMelting(int cost, String prefix, String name) {
    ...
    MeltingRecipeBuilder.melting(TinkerIngredients.of(item), result(cost * baseUnit), temperature, (float)Math.sqrt(cost))
  ...
  private void tagMelting(float scale, String output, float factor, Identifier tagName, boolean damagable, boolean forceOptional) {
    ...
    MeltingRecipeBuilder builder = MeltingRecipeBuilder.melting(TinkerIngredients.of(ItemTags.create(tagName)), result((int) (baseUnit * scale)), temperature, factor);
    if (damagable) {
      builder.setDamagable(damageUnits());
    }
    for (IByproduct byproduct : unitByproducts) {
  ...
  private void oreMelting(float scale, String tagPrefix, @Nullable TagKey<Item> size, float factor, String output, boolean forceOptional) {
    ...
    Ingredient baseIngredient = TinkerIngredients.of(itemTag(tagName));
    ...
    if (size == Tags.Items.ORE_RATES_SINGULAR) {
      ingredient = DifferenceIngredient.of(baseIngredient, TinkerIngredients.of(TinkerTags.Items.NON_SINGULAR_ORE_RATES));
      ...
      ingredient = IntersectionIngredient.of(baseIngredient, TinkerIngredients.of(size));
    ...
    if (oreByproducts.length == 0) {
      ...
      for (IByproduct byproduct : oreByproducts) {
        ...
        if (alwaysPresent) {
          builder.addCondition(AlwaysCondition.INSTANCE);
        } else {
  ...
  public SmelteryRecipeBuilder meltingCasting(float scale, String tagPrefix, String castMaterial, float factor, boolean forceOptional) {
    return meltingCasting(scale, tagPrefix, TinkerIngredients.of(itemTag(tagPrefix + "s/" + castMaterial)), factor, forceOptional);
  }
  ...
  public SmelteryRecipeBuilder metal() {
    ...
    blockCasting(9, TinkerIngredients.EMPTY, false);
  ...
  public SmelteryRecipeBuilder gem(int storageSize) {
    ...
    blockCasting(storageSize, TinkerIngredients.EMPTY, false);
