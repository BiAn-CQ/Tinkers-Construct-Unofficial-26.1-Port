...
public interface IMaterialRecipeHelper extends IRecipeHelper {
  ...
  default void metalMaterialRecipe(RecipeOutput consumer, MaterialVariantId material, String folder, String name, boolean optional) {
    ...
    materialRecipe(wrapped, material, TinkerIngredients.of(ingotTag), 1, 1, folder + matName + "/ingot");
    // nugget
    wrapped = optional ? withCondition(consumer, tagCondition("nuggets/" + name)) : consumer;
    materialRecipe(wrapped, material, TinkerIngredients.of(getItemTag(COMMON, "nuggets/" + name)), 1, 9, folder + matName + "/nugget");
    // block
    wrapped = optional ? withCondition(consumer, tagCondition("storage_blocks/" + name)) : consumer;
    materialRecipe(wrapped, material, TinkerIngredients.of(getItemTag(COMMON, "storage_blocks/" + name)), 9, 1, ItemOutput.fromTag(ingotTag), folder + matName + "/block");
  }
