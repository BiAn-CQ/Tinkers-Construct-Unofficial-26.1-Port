...
public class ContentModifier extends PageContent {
  ...
  public void load() {
    if (this.recipes == null) {
      ...
      if (modifier == ModifierManager.INSTANCE.getDefaultValue()) {
        ...
        this.recipes = slimeknights.tconstruct.library.utils.TinkerRecipeHelper.getJEIRecipes(level.registryAccess(), slimeknights.tconstruct.library.utils.TinkerRecipeHelper.getRecipeManager(level), TinkerRecipeTypes.TINKER_STATION.get(), IDisplayModifierRecipe.class).stream()
  ...
  public void buildAndAddRecipeDisplay(BookData book, ArrayList<BookElement> list, @Nullable IDisplayModifierRecipe recipe, @Nullable BookScreen parent) {
    if (recipe != null) {
      ...
      // The tool background must be emitted before the item.  Emitting it
      // after the item covers the dynamic tool with the slot texture.
