...
public abstract class AbstractTagInjectingTransformer<T> extends BookTransformer {
  ...
  private int addPages(SectionData section, JsonElement element, String key, int index) {
    try {
      ...
      Iterator<T> iterator = getTagEntries(TagKey.create(registry, JsonHelper.getIdentifier(load, "tag")));
      while (iterator.hasNext()) {
        ...
        if (section.source.resourceExists(section.source.getIdentifier(data))) {
          newPage.data = data;
        } else {
