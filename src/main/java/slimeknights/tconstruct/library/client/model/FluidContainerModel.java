...
public record FluidContainerModel(FluidStack fluid, boolean flipGas) implements IUnbakedGeometry<FluidContainerModel> {
  ...
  /** Parses the legacy model NBT field, accepting either a JSON object or an SNBT string. */
  private static CompoundTag parseLegacyNbt(JsonElement element) {
    String snbt = element.isJsonPrimitive() ? GsonHelper.convertToString(element, "nbt") : element.toString();
    try {
      return TagParser.parseTag(snbt);
    } catch (CommandSyntaxException e) {
      throw new JsonSyntaxException("Invalid fluid model NBT: " + snbt, e);
    }
  }
    ...
    if (json.has("fluid")) {
      ...
      if (fluidElement.isJsonObject()) {
        ...
        if (fluidObject.has("nbt")) {
          tag = parseLegacyNbt(fluidObject.get("nbt"));
        }
      } else {
        fluid = Loadables.FLUID.convert(fluidElement, "fluid");
      }
      fluidStack = slimeknights.tconstruct.library.utils.FluidStackDataUtil.create(fluid, FluidType.BUCKET_VOLUME, tag);
    }
  ...
  public BakedModel bake(IGeometryBakingContext context, ModelBaker bakery, Function<Material,TextureAtlasSprite> spriteGetter, ModelState modelState, ItemOverrides overrides) {
    ...
    if (modelLocation == null) {
      modelLocation = TConstruct.getResource("fluid_container_model");
    }
