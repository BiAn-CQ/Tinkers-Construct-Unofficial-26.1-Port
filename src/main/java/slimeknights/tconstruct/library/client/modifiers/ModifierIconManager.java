...
public class ModifierIconManager implements IEarlySafeManagerReloadListener {
  ...
  public static void init(AddClientReloadListenersEvent event) {
    event.addListener(TConstruct.getResource("modifier_icons"), INSTANCE);
  }
   ...
   * @param graphics  GUI extraction instance
    ...
    if (!icons.isEmpty()) {
      for (Identifier icon : icons) {
        TextureAtlasSprite sprite = Minecraft.getInstance().getAtlasManager()
          .get(new SpriteId(atlasFor(icon), icon));
        graphics.blitSprite(RenderPipelines.GUI_TEXTURED, sprite, x, y, size, size, -1);
      }
    } else {
      TextureAtlasSprite pages = Minecraft.getInstance().getAtlasManager()
        .get(new SpriteId(TextureAtlas.LOCATION_BLOCKS, DEFAULT_PAGES));
      TextureAtlasSprite cover = Minecraft.getInstance().getAtlasManager()
        .get(new SpriteId(TextureAtlas.LOCATION_BLOCKS, DEFAULT_COVER));
      graphics.blitSprite(RenderPipelines.GUI_TEXTURED, pages, x, y, size, size, -1);
      graphics.blitSprite(RenderPipelines.GUI_TEXTURED, cover, x, y, size, size, 0xFF000000 | modifier.getColor());
    }
  }

  /** Item textures moved to their own atlas in 26.1; GUI and block textures remain on the block atlas. */
  private static Identifier atlasFor(Identifier icon) {
    return icon.getPath().startsWith("item/") ? TextureAtlas.LOCATION_ITEMS : TextureAtlas.LOCATION_BLOCKS;
  }
}
