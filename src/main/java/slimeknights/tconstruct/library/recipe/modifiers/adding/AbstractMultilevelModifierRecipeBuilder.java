...
public abstract class AbstractMultilevelModifierRecipeBuilder<T extends AbstractMultilevelModifierRecipeBuilder<T>> extends AbstractRecipeBuilder<T> {
  ...
  protected Ingredient tools = TinkerIngredients.EMPTY;
  ...
  public T setTools(TagKey<Item> tag) {
    return this.setTools(TinkerIngredients.of(tag));
  }
  ...
  public T saveSalvage(RecipeOutput consumer, Identifier id) {
    if (levels.isEmpty()) {
      throw new IllegalStateException("Must have at least 1 level");
    }
    for (LevelEntry levelEntry : levels) {
      if (levelEntry.slots() != null) {
        Identifier recipeId = id.withSuffix("_level_" + levelEntry.level().min());
        saveRecipe(consumer, recipeId, new ModifierSalvage(
          recipeId, tools, maxToolSize, result, levelEntry.level(), levelEntry.slots()), null);
      }
    }
    return self();
  }
