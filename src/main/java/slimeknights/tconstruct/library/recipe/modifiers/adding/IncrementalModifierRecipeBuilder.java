...
public class IncrementalModifierRecipeBuilder extends AbstractModifierRecipeBuilder<IncrementalModifierRecipeBuilder> {
  private Ingredient input = TinkerIngredients.EMPTY;
  ...
  public IncrementalModifierRecipeBuilder setInput(ItemLike item, int amountPerItem, int neededPerLevel) {
    return setInput(TinkerIngredients.of(item), amountPerItem, neededPerLevel);
  }
  ...
  public IncrementalModifierRecipeBuilder setInput(TagKey<Item> tag, int amountPerItem, int neededPerLevel) {
    return setInput(TinkerIngredients.of(tag), amountPerItem, neededPerLevel);
  }
  ...
  public void save(RecipeOutput consumer, Identifier id) {
    if (input == TinkerIngredients.EMPTY) {
      throw new IllegalStateException("Must set input");
    }
    var advancementId = buildOptionalAdvancement(id, "modifiers");
    saveRecipe(consumer, id, new IncrementalModifierRecipe(id, input, amountPerItem, neededPerLevel, tools, maxToolSize, result, ModifierEntry.VALID_LEVEL.range(minLevel, maxLevel), slots, leftover, allowCrystal, checkTraitLevel), advancementId);
  }
}
