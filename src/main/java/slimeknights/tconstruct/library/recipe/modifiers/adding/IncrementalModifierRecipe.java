...
public class IncrementalModifierRecipe extends AbstractModifierRecipe {
  ...
  protected List<List<ItemStack>> getInputs() {
    if (slotCache == null) {
      ...
      List<ItemStack> items = Arrays.asList(slimeknights.tconstruct.library.recipe.TinkerIngredients.getItems(input));
      ...
      Lazy<List<ItemStack>> fullSize = Lazy.of(() -> items.stream().map(stack -> slimeknights.tconstruct.library.utils.ItemStackDataUtil.copyStackWithSize(stack, maxStackSize)).collect(Collectors.toList()));
      ...
      if (needed > 0) {
        ...
        builder.add(items.stream().map(stack -> slimeknights.tconstruct.library.utils.ItemStackDataUtil.copyStackWithSize(stack, remaining)).collect(Collectors.toList()));
      }
  ...
  public static void updateInputs(IMutableTinkerStationContainer inv, Ingredient ingredient, int amountNeeded, int amountPerInput, ItemStack leftover) {
    ...
    if (leftoverAmount > 0) {
      ...
      if (!leftover.isEmpty()) {
        ...
        inv.giveItem(slimeknights.tconstruct.library.utils.ItemStackDataUtil.copyStackWithSize(leftover, (amountPerInput - leftoverAmount) * leftover.getCount()));
      }
    }
    for (int i = 0; i < inv.getInputCount(); i++) {
      ItemStack stack = inv.getInput(i);
