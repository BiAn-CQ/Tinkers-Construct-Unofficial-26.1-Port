...
public class ModifierRepairTinkerStationRecipe implements ITinkerStationRecipe, IModifierRepairRecipe, IMultiRecipe<IDisplayToolTinkering> {
  ...
  public ModifierRepairTinkerStationRecipe(Identifier id, ModifierId modifier, Ingredient ingredient, int repairAmount) {
    this.id = id;
  ...
  @Getter
  static class DisplayRecipe extends AbstractCraftingTinkeringRecipe {
    private final Identifier id;
    private final ModifierId modifier;
    private final int repairAmount;
    ...
    public DisplayRecipe(Identifier id, IModifierRepairRecipe recipe, boolean isCrafting) {
      ...
      this.inputs = List.of(slimeknights.tconstruct.library.recipe.TinkerIngredients.getItems(recipe.getIngredient()));
      ...
      toolWithoutModifier = RegistryHelper.getTagValueStream(BuiltInRegistries.ITEM, TinkerTags.Items.DURABILITY)
        .map(item -> {
          if (item instanceof IModifiableDisplay modifiable) {
            ...
            slimeknights.tconstruct.library.utils.ItemStackDataUtil.updateTag(stack, tag -> {
      ...
      toolWithModifier = toolWithoutModifier.stream().map(stack -> {
        stack = stack.copy();
        slimeknights.tconstruct.library.utils.ItemStackDataUtil.updateTag(stack, tag -> tag.putInt(ToolStack.TAG_DAMAGE, 200 - repairAmount));
        return stack;
      }).toList();
    }
