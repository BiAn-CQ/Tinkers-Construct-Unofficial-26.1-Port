...
public class ToolBuildingRecipe implements ITinkerStationRecipe {
  ...
  public boolean matches(ITinkerStationContainer inv, Level worldIn) {
    ...
    if (requiredInputs == 1 && partSize == 0) {
      ...
      for (int i = 0; i < maxInputs; i++) {
        ...
        if (!stack.isEmpty()) {
          ...
          if (found || !TinkerIngredients.matches(ingredient, stack)) {
            return false;
          }
          found = true;
        }
    ...
    for (; i < maxInputs; i++) {
      Ingredient ingredient = LogicHelper.getOrDefault(ingredients, i - partSize, TinkerIngredients.EMPTY);
      if (!TinkerIngredients.matches(ingredient, inv.getInput(i))) {
        return false;
      }
    }
      ...
      if (!this.materials.isEmpty()) {
        ...
        if (offset < materialCount) {
          ...
          if (offset == 0) {
            ...
            ItemStackDataUtil.updateTag(result, tag -> tag.putBoolean(TooltipUtil.KEY_DISPLAY, true));
          }
        }
      }
