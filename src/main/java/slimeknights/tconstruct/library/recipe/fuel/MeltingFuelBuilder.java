...
public class MeltingFuelBuilder extends AbstractRecipeBuilder<MeltingFuelBuilder> {
  ...
  public static MeltingFuelBuilder fuel(FluidStack fluid, int duration) {
    return fuel(FluidTinkerIngredients.of(fluid), duration, getTemperature(fluid));
  }

  /** Setups the builder for solid fuel */
  @Internal
  public static MeltingFuelBuilder solid(int temperature) {
    return fuel(FluidTinkerIngredients.EMPTY, 0, temperature);
  }
  ...
  public void save(RecipeOutput consumer, Identifier id) {
    var advancementId = this.buildOptionalAdvancement(id, "melting_fuel");
    saveRecipe(consumer, id, new MeltingFuel(id, input, duration, temperature, rate), advancementId);
  }
}
