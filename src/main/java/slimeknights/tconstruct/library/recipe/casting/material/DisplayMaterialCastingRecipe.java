...
public sealed abstract class DisplayMaterialCastingRecipe implements IDisplayableCastingRecipe {
  ...
  private DisplayMaterialCastingRecipe(@Nullable Identifier recipeId, RecipeType<?> type, List<ItemStack> castItems, List<FluidStack> fluids, List<ItemStack> outputs, int maxCoolingTime, boolean consumed) {
  ...
  private static final class Casting extends DisplayMaterialCastingRecipe {
    ...
    private Casting(@Nullable Identifier recipeId, RecipeType<?> type, List<ItemStack> castItems, List<FluidStack> fluids, List<ItemStack> outputs, Object2IntMap<Fluid> coolingTimes, int maxCoolingTime, boolean consumed) {
    ...
    public int getCoolingTime(FluidStack fluid) {
      return coolingTimes.getOrDefault(fluid.getFluid(), coolingTime);
    }
  }
  ...
  private static final class Composite extends DisplayMaterialCastingRecipe {
    ...
    private Composite(@Nullable Identifier recipeId, RecipeType<?> type, List<ItemStack> castItems, List<FluidStack> fluids, List<ItemStack> outputs, Object2IntMap<CompositeFluid> coolingTimes, int maxCoolingTime, boolean consumed) {
      super(recipeId, type, castItems, fluids, outputs, maxCoolingTime, consumed);
      this.coolingTimes = coolingTimes;
    }
  ...
  public static Builder from(ICastingRecipe recipe) {
    return type(recipe.getType()).id(recipe instanceof IDisplayableCastingRecipe display ? display.getRecipeId() : null);
  }
  ...
  public static class Builder {
    ...
    public Builder cast(Ingredient cast) {
      return casts(List.of(slimeknights.tconstruct.library.recipe.TinkerIngredients.getItems(cast)));
    }
