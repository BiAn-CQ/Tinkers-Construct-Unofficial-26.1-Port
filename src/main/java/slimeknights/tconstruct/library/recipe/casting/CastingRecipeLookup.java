...
public class CastingRecipeLookup {
  ...
  public static void registerCastable(ItemOutput output) {
    ...
    // item name output fails here, but that only happens at datagen so just ignore that
      ...
      // ItemStackTemplate-backed outputs are decoded while item component holders are
      // still being bound. Resolving one here calls ItemStack's constructor too early.
      // The item identity is sufficient for the castable lookup and is safe to read now.
