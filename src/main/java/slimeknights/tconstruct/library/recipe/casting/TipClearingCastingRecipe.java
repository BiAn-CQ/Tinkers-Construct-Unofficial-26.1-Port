...
public class TipClearingCastingRecipe extends PotionCastingRecipe {
  ...
  public List<DisplayCastingRecipe> getRecipes(HolderLookup.Provider access) {
    if (displayRecipes == null) {
      ...
      List<ItemStack> toolWithPotion = BuiltInRegistries.POTION.stream().map(BuiltInRegistries.POTION::wrapAsHolder)
        .flatMap(potion -> {
      ...
      List<ItemStack> toolWithoutPotion = BuiltInRegistries.POTION.stream().map(BuiltInRegistries.POTION::wrapAsHolder)
