...
public class PartSwapCastingRecipe extends AbstractMaterialCastingRecipe implements IMultiRecipe<IDisplayableCastingRecipe> {
  ...
  public boolean matches(ICastingContainer inv, Level level) {
    return TinkerIngredients.matches(getCast(), inv.getStack()) && canPartSwap(inv);
  }

  @Override
  public ItemStack getResultItem(HolderLookup.Provider registryAccess) {
    return slimeknights.tconstruct.library.recipe.TinkerIngredients.getItems(getCast())[0].copy();
  }
  ...
  protected static void setDisplayMaterials(ToolStack tool, int index, MaterialVariant material) {
  ...
  static boolean supportsDisplayMaterials(MaterialStatsId stats, FluidRecipe recipe, boolean composite) {
    return stats.canUseMaterial(recipe.output.getId()) && (!composite || stats.canUseMaterial(recipe.input.getId()));
  }
    ...
    if (!fluidRecipes.isEmpty()) {
      ...
      for (ToolRequirement tool : tools) {
        ...
        List<FluidRecipe> filtered = fluidRecipes.stream()
          .filter(recipe -> supportsDisplayMaterials(tool.requirement, recipe, composite))
          .toList();
        if (!filtered.isEmpty()) {
  ...
  public List<IDisplayableCastingRecipe> getRecipes(HolderLookup.Provider access) {
    if (multiRecipes == null) {
      List<ToolRequirement> tools = getTools(TinkerIngredients.getItems(getCast()));
      if (tools.isEmpty()) {
