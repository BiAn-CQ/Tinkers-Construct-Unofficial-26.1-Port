...
public class ToolCastingRecipe extends PartSwapCastingRecipe implements IMultiRecipe<IDisplayableCastingRecipe> {
  ...
  protected int getIndex(List<MaterialStatsId> requirements) {
    return castPurpose == CastPurpose.MAYBE_MATERIAL ? requirements.size() - 1 : castPurpose.fluidIndex;
  }
  ...
  /** @deprecated use {@link #ToolCastingRecipe(TypeAwareRecipeSerializer, Identifier, String, Ingredient, int, CastPurpose, IModifiable, IJsonPredicate, List, boolean)} */
  ...
  public boolean matches(ICastingContainer inv, Level level) {
    ...
    if (!slimeknights.tconstruct.library.recipe.TinkerIngredients.matches(this.getCast(), cast)) {
      return false;
    }
  ...
  private class DisplayRecipe implements IDisplayableCastingRecipe {
    ...
    private DisplayRecipe(int fluidIndex, MaterialStatsId statType, List<ItemStack> castItems, List<FluidStack> fluids, int coolingTime) {
      ...
      this.castItem = !castItems.isEmpty() && castItems.get(0).getItem() instanceof IMaterialItem item ? item : null;
    }
