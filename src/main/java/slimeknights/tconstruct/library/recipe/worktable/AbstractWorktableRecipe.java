...
public abstract class AbstractWorktableRecipe implements IModifierWorktableRecipe {
  public static final Ingredient DEFAULT_TOOLS = TinkerIngredients.of(TinkerTags.Items.MODIFIABLE);
  ...
  public AbstractWorktableRecipe(Identifier id, List<SizedIngredient> inputs) {
    this(id, TinkerIngredients.of(TinkerTags.Items.MODIFIABLE), inputs);
  }
  ...
  public List<ItemStack> getInputTools() {
    if (tools == null) {
      tools = Arrays.stream(slimeknights.tconstruct.library.recipe.TinkerIngredients.getItems(toolRequirement)).map(stack -> IModifiableDisplay.getDisplayStack(stack.getItem())).toList();
    }
    return tools;
  }
