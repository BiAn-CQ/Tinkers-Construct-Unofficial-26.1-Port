...
public class ModifierSetWorktableRecipe extends AbstractWorktableRecipe {
  ...
  public static final BiFunction<CompoundTag, String, ListTag> LIST_GETTER = CompoundTag::getListOrEmpty;
  /** Loader instance */
  ...
  public static Set<ModifierId> getModifierSet(IModDataView modData, Identifier key) {
    return modData.get(key, LIST_GETTER).stream().map(tag -> ModifierId.tryParse(stringValue(tag))).filter(Objects::nonNull).collect(Collectors.toSet());
  }
  ...
  public static boolean isInSet(ListTag list, ModifierId modifier, boolean remove) {
    ...
    while (iterator.hasNext()) {
      if (modifierStr.equals(stringValue(iterator.next()))) {
        if (remove) {
          iterator.remove();
        }
        return true;
