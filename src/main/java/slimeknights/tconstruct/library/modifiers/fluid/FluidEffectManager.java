...
public class FluidEffectManager extends TinkerJsonResourceReloadListener {
  /** Recipe folder */
  ...
  private static final FluidEffects EMPTY = new FluidEffects(FluidTinkerIngredients.EMPTY, List.of(), List.of(), true);

  /** Condition context for recipe loading */
  private IContext conditionContext = IContext.EMPTY;
  private RegistryAccess registryAccess;
  private RegistryOps.RegistryInfoLookup registryLookup;
  ...
  private void addDataPackListeners(final AddServerReloadListenersEvent event) {
    ...
    registryLookup = RegistryOps.create(JsonOps.INSTANCE, event.getServerResources().getRegistryLookup()).lookupProvider;
  }
  ...
  protected void apply(Map<Identifier,JsonElement> splashList, ResourceManager pResourceManager, ProfilerFiller pProfiler) {
    ...
    for (Entry<Identifier,JsonElement> entry : splashList.entrySet()) {
      ...
      try {
        ...
        if (!slimeknights.tconstruct.library.json.condition.ConditionUtil.testConditions(json, "conditions", conditionContext)) {
          continue;
        }
        TypedMapBuilder context = contextBuilder(key).put(ContextKey.CONDITION_CONTEXT, conditionContext);
        if (registryAccess != null) {
          context.put(TinkerContextKeys.REGISTRY_ACCESS, registryAccess);
        }
        if (registryLookup != null) {
          context.put(ContextKey.REGISTRY_LOOKUP, registryLookup);
        }
        fluids.add(new FluidEffects.Entry(key, FluidEffects.LOADABLE.deserialize(json, context.build())));
