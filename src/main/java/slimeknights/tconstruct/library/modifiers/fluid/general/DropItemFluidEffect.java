...
public record DropItemFluidEffect(ItemOutput item) implements FluidEffect<FluidEffectContext> {
  ...
  public float apply(FluidStack fluid, EffectLevel level, FluidEffectContext context, SimulationMode action) {
    ...
    if (count > 0) {
      if (action.execute()) {
        ModifierUtil.dropItem(context.getLevel(), context.getLocation(), slimeknights.tconstruct.library.utils.ItemStackDataUtil.copyStackWithSize(item.get(), count * item.getCount()));
      }
