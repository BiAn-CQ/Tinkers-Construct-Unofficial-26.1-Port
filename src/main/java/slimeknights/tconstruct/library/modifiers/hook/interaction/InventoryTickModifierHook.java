...
public interface InventoryTickModifierHook {
  /**
   * Converts the nullable equipment slot used by the 26.1 item tick callback to
   * the legacy integer slot expected by modifier hooks. A null slot represents a
   * normal inventory entry, which has no equipment index.
   */
  static int getItemSlot(EquipmentSlot slot) {
    return slot == null ? -1 : slot.getIndex();
  }

  /** Returns whether the nullable callback slot identifies a selected hand item. */
  static boolean isSelected(EquipmentSlot slot) {
    return slot != null && slot.getType() == EquipmentSlot.Type.HAND;
  }
