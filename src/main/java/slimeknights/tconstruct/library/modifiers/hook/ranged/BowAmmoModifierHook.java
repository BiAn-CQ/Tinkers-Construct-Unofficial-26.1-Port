...
public interface BowAmmoModifierHook {
  ...
  static ItemStack getAmmo(IToolStackView tool, ItemStack bow, LivingEntity living, @Nullable Predicate<ItemStack> predicate) {
    ...
    if (tool.getVolatileData().getBooleanOr(SKIP_INVENTORY_AMMO, false)) {
  ...
  static int getDesiredProjectiles(IToolStackView tool) {
    return 1 + tool.getVolatileData().getIntOr(MULTISHOT, 0);
  }
  ...
  static ItemStack consumeAmmo(IToolStackView tool, ItemStack bow, LivingEntity living, @Nullable Player player, @Nullable Predicate<ItemStack> predicate, int projectilesDesired) {
    ...
    boolean skipInventoryAmmo = tool.getVolatileData().getBooleanOr(SKIP_INVENTORY_AMMO, false);
    ItemStack standardAmmo;
    ...
    if (predicate != null) {
      for (ModifierEntry entry : tool.getModifierList()) {
        ...
        if (!ammo.isEmpty()) {
          ...
          if (creative) {
            return slimeknights.tconstruct.library.utils.ItemStackDataUtil.copyStackWithSize(ammo, projectilesDesired);
          }

          // not creative, split out the desired amount. We may have to do more work if it is too small
          resultStack = slimeknights.tconstruct.library.utils.ItemStackDataUtil.copyStackWithSize(ammo, Math.min(projectilesDesired, ammo.getCount()));
    ...
    if (resultStack.isEmpty()) {
      ...
      if (creative) {
        return slimeknights.tconstruct.library.utils.ItemStackDataUtil.copyStackWithSize(standardAmmo, projectilesDesired);
      }
