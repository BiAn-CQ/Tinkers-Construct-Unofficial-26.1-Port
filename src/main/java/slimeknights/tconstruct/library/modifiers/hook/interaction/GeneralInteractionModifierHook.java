...
public interface GeneralInteractionModifierHook {
  ...
  static void addCooldown(IToolStackView tool, Player player, float speedFactor) {
    player.getCooldowns().addCooldown(BuiltInRegistries.ITEM.getKey(tool.getItem()), getDrawtime(tool, player, speedFactor));
  }
  ...
  static float getToolCharge(IToolStackView tool, float chargeTime) {
    ...
    float charge = chargeTime / tool.getPersistentData().getIntOr(KEY_DRAWTIME, 0);
  ...
  static ModifierEntry getActiveModifier(IToolStackView tool) {
    ...
    if (persistentData.contains(KEY_ACTIVE_MODIFIER)) {
      ModifierId modifier = ModifierId.tryParse(persistentData.getStringOr(KEY_ACTIVE_MODIFIER, ""));
      if (modifier != null) {
