...
public interface EnchantmentModule extends ModifierModule, LevelingIntModule, ConditionalModule<IToolStackView> {
  /* Common fields */
  LoadableField<Holder<Enchantment>,EnchantmentModule> ENCHANTMENT = TinkerLoadables.ENCHANTMENT_HOLDER.requiredField("name", EnchantmentModule::enchantment);
  ...
  class Protection extends Constant implements ProtectionModifierHook {
    ...
    public float getProtectionModifier(IToolStackView tool, ModifierEntry modifier, EquipmentContext context, EquipmentSlot slotType, DamageSource source, float modifierValue) {
      if (condition().matches(tool, modifier)) {
        ...
        if (subtractLevel > 0 && enchantment.matchingSlot(slotType) && !source.is(DamageTypeTags.BYPASSES_ENCHANTMENTS) && context.getLevel() instanceof ServerLevel server) {
          MutableFloat protection = new MutableFloat();
          enchantment.modifyDamageProtection(server, subtractLevel, context.getEntity().getItemBySlot(slotType), context.getEntity(), source, protection);
   ...
   */
  record MainHandHarvest(Holder<Enchantment> enchantment, LevelingInt level, ModifierCondition<IToolStackView> condition, Identifier conditionFlag, IJsonPredicate<BlockState> block, IJsonPredicate<LivingEntity> holder) implements EnchantmentModule, EnchantmentModifierHook, BlockHarvestModifierHook {
    ...
    @Deprecated(forRemoval = true)
    public MainHandHarvest(Holder<Enchantment> enchantment, int level, ModifierCondition<IToolStackView> condition, Identifier conditionFlag, IJsonPredicate<BlockState> block, IJsonPredicate<LivingEntity> holder) {
    ...
    @Override
    public int updateEnchantmentLevel(IToolStackView tool, ModifierEntry modifier, Holder<Enchantment> enchantment, int level) {
      if (enchantment.equals(this.enchantment()) && tool.getPersistentData().getBooleanOr(conditionFlag, false)) {
        level += getLevel(modifier);
      }
      return level;
    }
    ...
    @Override
    public void updateEnchantments(IToolStackView tool, ModifierEntry modifier, Map<Holder<Enchantment>,Integer> map) {
      if (tool.getPersistentData().getBooleanOr(conditionFlag, false)) {
        EnchantmentModifierHook.addEnchantment(map, this.enchantment(), getLevel(modifier));
      }
    }
