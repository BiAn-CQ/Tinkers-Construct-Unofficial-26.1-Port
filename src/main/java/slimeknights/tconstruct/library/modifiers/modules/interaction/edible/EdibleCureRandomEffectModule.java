...
/** Removes one random curable effect after eating. */
public record EdibleCureRandomEffectModule(IJsonPredicate<LivingEntity> holder, ModifierCondition<IToolStackView> condition)
  ...
  public static final RecordLoadable<EdibleCureRandomEffectModule> LOADER = RecordLoadable.create(
    LivingEntityPredicate.LOADER.defaultField("holder", EdibleCureRandomEffectModule::holder), ModifierCondition.TOOL_FIELD, EdibleCureRandomEffectModule::new);
  public EdibleCureRandomEffectModule() { this(LivingEntityPredicate.ANY, ModifierCondition.ANY_TOOL); }
  @Override public RecordLoadable<EdibleCureRandomEffectModule> getLoader() { return LOADER; }
  ...
  @Override public void onToolEaten(IToolStackView tool, ModifierEntry modifier, Player player, EquipmentSlot eatenSlot, int hunger, float saturation, List<ItemStack> items) {
    if (condition.matches(tool, modifier) && holder.matches(player)) CheeseItem.removeRandomEffect(player);
  }
}
