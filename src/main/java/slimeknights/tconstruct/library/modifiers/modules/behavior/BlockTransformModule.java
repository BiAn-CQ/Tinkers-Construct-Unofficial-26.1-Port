...
public interface BlockTransformModule extends ModifierModule, BlockInteractionModifierHook, AreaOfEffectHighlightModifierHook {
  ...
  default InteractionResult afterBlockUse(IToolStackView tool, ModifierEntry modifier, UseOnContext context, InteractionSource source) {
          ...
          if (transform(tool, offsetContext, newTarget, totalTransformed < 40)) {
            ...
            if (ToolDamageUtil.damage(tool, 1, player, stack, modifier.getId())) {
              if (player != null) {
                ...
                player.onEquippedItemBroken(context.getItemInHand().getItem(), slot);
              }
              break;
            }
          }
    ...
    return didTransform ? (world.isClientSide() ? InteractionResult.SUCCESS : InteractionResult.SUCCESS_SERVER) : InteractionResult.PASS;
  }
