 ...
 */
public record InfinityModule(ItemStackTemplate ammo, String variantTag, int durabilityUsage, boolean checkStandardArrows) implements ModifierModule, BowAmmoModifierHook, ModifierRemovalHook, ProjectileLaunchModifierHook.NoShooter {
  ...
  public InfinityModule(ItemStack ammo, String variantTag, int durabilityUsage, boolean checkStandardArrows) {
    this(ItemStackTemplate.fromNonEmptyStack(ammo), variantTag, durabilityUsage, checkStandardArrows);
  }
  ...
  public ItemStack findAmmo(IToolStackView tool, ModifierEntry modifier, LivingEntity shooter, ItemStack standardAmmo, Predicate<ItemStack> ammoPredicate) {
    ...
    ItemStack ammo = this.ammo.create().copyWithCount(count);
    ItemStackDataUtil.updateTag(ammo, tag -> {
  ...
  public void onProjectileShoot(IToolStackView tool, ModifierEntry modifier, @Nullable LivingEntity shooter, ItemStack ammo, Projectile projectile, @Nullable AbstractArrow arrow, ModDataNBT persistentData, boolean primary) {
    ...
    if (arrow != null && arrow.pickup != Pickup.CREATIVE_ONLY) {
      var tag = ItemStackDataUtil.getTag(ammo);
