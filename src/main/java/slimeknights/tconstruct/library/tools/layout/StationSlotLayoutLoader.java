 ...
 */
@Log4j2
public class StationSlotLayoutLoader extends TinkerJsonResourceReloadListener {
  ...
  protected void apply(Map<Identifier,JsonElement> splashList, ResourceManager resourceManager, ProfilerFiller profiler) {
    ...
    for (Entry<Identifier,JsonElement> entry : splashList.entrySet()) {
      ...
      try {
        ...
        if (!object.entrySet().isEmpty() && slimeknights.tconstruct.library.json.condition.ConditionUtil.testConditions(object, "conditions", conditionContext)) {
  ...
  private static class IngredientSerializer implements JsonSerializer<Ingredient>, JsonDeserializer<Ingredient> {
    ...
    public Ingredient deserialize(JsonElement json, Type typeOfT, JsonDeserializationContext context) throws JsonParseException {
      return IngredientLoadable.DISALLOW_EMPTY.convert(json, "ingredient", TypedMap.empty());
    }
