...
public class LayoutSlot {
  ...
  public static LayoutSlot read(FriendlyByteBuf buffer) {
    ...
    if (buffer.readBoolean()) {
      ingredient = Ingredient.CONTENTS_STREAM_CODEC.decode(TinkerNetworkBuffer.registry(buffer));
    }
  ...
  public void write(FriendlyByteBuf buffer) {
    if (icon != null) {
      ...
      buffer.writeIdentifier(icon.location());
    } else {
    ...
    if (filter != null) {
      buffer.writeBoolean(true);
      Ingredient.CONTENTS_STREAM_CODEC.encode(TinkerNetworkBuffer.registry(buffer), filter);
    } else {
      buffer.writeBoolean(false);
    }
  }
