...
public interface BlockItemProviderCapability {
  ...
  static void register() {
    net.neoforged.fml.ModLoadingContext.get().getActiveContainer().getEventBus().addListener(RegisterCapabilitiesEvent.class, BlockItemProviderCapability::register);
  }
  ...
  private static void register(RegisterCapabilitiesEvent event) {
    for (var item : BuiltInRegistries.ITEM) {
      if (item instanceof BlockItem) {
        event.registerItem(CAPABILITY, (stack, context) -> SimpleBlockItem.INSTANCE, item);
      }
    }
  }
