...
public class ToolTankHelper {
  ...
  public static final BiFunction<CompoundTag, String, FluidStack> PARSE_FLUID =
    (nbt, key) -> FluidStackDataUtil.parse(RegistryAccessUtil.getRegistryAccess(), nbt.getCompoundOrEmpty(key));
  ...
  public int getCapacity(IToolStackView tool) {
    return tool.getStats().getIntOr(capacityStat, 0);
  }
  ...
  public FluidStack setFluid(IToolStackView tool, FluidStack fluid) {
    ...
    tool.getPersistentData().put(fluidKey, FluidStackDataUtil.save(RegistryAccessUtil.getRegistryAccess(), fluid));
    return fluid;
  }
}
