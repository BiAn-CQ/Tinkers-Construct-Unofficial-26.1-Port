...
public record CircleWeaponAttack(LevelingValue diameters) implements MeleeHitToolHook, ToolModule {
  ...
  public void afterMeleeHit(IToolStackView tool, ToolAttackContext context, float damage) {
    ...
    double range = diameters.compute(tool.getVolatileData().getIntOr(IModifiable.EXPANDED, 0));
    ...
    if (range > 0) {
      ...
      if (!context.isProjectile() && player != null && level instanceof ServerLevel serverLevel) {
        double dx = -Mth.sin(player.getYRot() * ((float) Math.PI / 180F));
        double dz = Mth.cos(player.getYRot() * ((float) Math.PI / 180F));
        serverLevel.sendParticles(ParticleTypes.SWEEP_ATTACK,
          player.getX() + dx, player.getY(0.5), player.getZ() + dz, 0, dx, 0, dz, 0);
      }
    }
  }
}
