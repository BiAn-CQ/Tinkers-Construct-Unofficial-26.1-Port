...
public record SweepWeaponAttack(LevelingValue ranges) implements MeleeHitToolHook, ToolModule {
  ...
  public static float getSweepingDamage(IToolStackView tool, float baseDamage) {
    float percent = tool.getVolatileData().getFloatOr(SWEEP_PERCENT, 0.0f);
  ...
  public void afterMeleeHit(IToolStackView tool, ToolAttackContext context, float damage) {
    ...
    if (context.isFullyCharged() && !attacker.isSprinting() && !context.isCritical() && !context.isProjectile() && attacker.onGround() && attacker.walkAnimation.speed() < attacker.getSpeed()) {
      // loop through all nearby entities
      double range = this.ranges.compute(tool.getVolatileData().getIntOr(IModifiable.EXPANDED, 0));
      ...
      for (LivingEntity aoeTarget : level.getEntitiesOfClass(LivingEntity.class, target.getBoundingBox().inflate(range, 0.25D, range))) {
        if (aoeTarget != attacker && aoeTarget != target && !attacker.isAlliedTo(aoeTarget)
          ...
          aoeTarget.hurtOrSimulate(source, sweepDamage);
        }
      }
