...
public class ToolStack implements IToolStackView {
  ...
  private void syncStack() {
    if (backingStack != null) {
      ...
      updateDynamicComponents(backingStack);
    }
  }

  /** Updates vanilla components whose presence depends on rebuilt tool data. */
  private void updateDynamicComponents(ItemStack stack) {
    if (item instanceof IModifiable modifiable) {
      modifiable.updateDynamicComponents(stack);
    }
  }
  ...
  protected void breakTool() {
    setDamage(getStats().getIntOr(ToolStats.DURABILITY, 0));
  }
  ...
  public int getDamage() {
    ...
    int durability = getStats().getIntOr(ToolStats.DURABILITY, 0);
    if (isBroken()) {
      return durability;
    }
  ...
  public int getCurrentDurability() {
    ...
    return Math.max(0, getStats().getIntOr(ToolStats.DURABILITY, 0) - getDamageRaw());
  }
  ...
  protected void setStats(StatsNBT stats) {
    ...
    int newMax = stats.getIntOr(ToolStats.DURABILITY, 0);
  ...
  public ToolDataNBT getPersistentData() {
    ...
    if (persistentModData == null) {
      ...
      if (nbt.contains(TAG_PERSISTENT_MOD_DATA)) {
        persistentModData = ToolDataNBT.readFromNBT(nbt.getCompoundOrEmpty(TAG_PERSISTENT_MOD_DATA), this::syncStack);
      } else {
