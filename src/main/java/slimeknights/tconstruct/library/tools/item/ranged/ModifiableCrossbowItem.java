...
public class ModifiableCrossbowItem extends ModifiableLauncherItem {
  ...
  public InteractionResult use(Level level, Player player, InteractionHand hand) {
    ...
    CompoundTag heldAmmo = persistentData.getCompoundOrEmpty(KEY_CROSSBOW_AMMO);
    if (heldAmmo.isEmpty()) {
      ...
      if (!ammo.isEmpty() || tool.getModifiers().has(TinkerTags.Modifiers.CHARGE_EMPTY_BOW_WITH_DRAWTIME)) {
        ...
        if (!ammo.isEmpty()) {
          if (storeDrawingItem) {
            persistentData.put(KEY_DRAWBACK_AMMO, ItemStackDataUtil.save(level.registryAccess(), ammo));
          } else {
  ...
  public static void fireCrossbow(IToolStackView tool, LivingEntity living, boolean creative, InteractionHand hand, CompoundTag heldAmmo) {
    ...
    if (!level.isClientSide()) {
      ...
      ItemStack weapon = living.getItemInHand(hand);
      ItemStack ammo = ItemStackDataUtil.parse(living.registryAccess(), heldAmmo);
      ...
      for (int arrowIndex = 0; arrowIndex < ammo.getCount(); arrowIndex++) {
        ...
        if (ammo.is(Items.FIREWORK_ROCKET)) {
          ...
          arrow = arrowItem.createArrow(level, ammo, living, weapon);
          projectile = arrow;
          ...
          float baseArrowDamage = tool.getStats().get(ToolStats.PROJECTILE_DAMAGE);
  ...
  public boolean releaseUsing(ItemStack bow, Level level, LivingEntity living, int chargeRemaining) {
    ...
    ModDataNBT persistentData = tool.getPersistentData();
    if (tool.isBroken() || getUseDuration(bow, living) - chargeRemaining < persistentData.getIntOr(KEY_DRAWTIME, 0) || persistentData.contains(KEY_CROSSBOW_AMMO)) {
      return false;
    }
    ...
    if (!ammo.isEmpty()) {
      ...
      if (!level.isClientSide()) {
        CompoundTag ammoNBT = ItemStackDataUtil.save(level.registryAccess(), ammo);
  ...
  public List<Component> getStatInformation(IToolStackView tool, @Nullable Player player, List<Component> tooltips, TooltipKey key, TooltipFlag tooltipFlag) {
    ...
    CompoundTag heldAmmo = tool.getPersistentData().getCompoundOrEmpty(KEY_CROSSBOW_AMMO);
    if (!heldAmmo.isEmpty()) {
      ItemStack heldStack = player == null ? ItemStack.EMPTY : ItemStackDataUtil.parse(player.registryAccess(), heldAmmo);
      if (!heldStack.isEmpty()) {
        ...
        if (tooltipFlag.isAdvanced() && player != null) {
          ...
          nestedTooltip.addAll(heldStack.getTooltipLines(Item.TooltipContext.of(player.level()), player, tooltipFlag));
