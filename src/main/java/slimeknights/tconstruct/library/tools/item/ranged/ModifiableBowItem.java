...
public class ModifiableBowItem extends ModifiableLauncherItem {
  ...
  public static boolean isBallista(IToolStackView tool) {
    return tool.hasTag(TinkerTags.Items.BALLISTAS) && tool.getVolatileData().getBooleanOr(KEY_BALLISTA, false);
  }
  ...
  public InteractionResult use(Level level, Player player, InteractionHand hand) {
    ...
    InteractionResult override = EventHooks.onArrowNock(bow, level, player, hand, !ammo.isEmpty());
    if (override != null) {
      return override;
    }
    ...
    if (!ammo.isEmpty()) {
      if (storeDrawingItem) {
        tool.getPersistentData().put(KEY_DRAWBACK_AMMO, ItemStackDataUtil.save(player.registryAccess(), ammo));
      } else {
  ...
  public boolean releaseUsing(ItemStack bow, Level level, LivingEntity living, int timeLeft) {
    ...
    Predicate<ItemStack> ammoPredicate = switch (tool.getPersistentData().getIntOr(KEY_BALLISTA, 0)) {
    ...
    boolean hasAmmo = !foundAmmo.isEmpty() || creative && !tool.getVolatileData().getBooleanOr(BowAmmoModifierHook.SKIP_INVENTORY_AMMO, false);
