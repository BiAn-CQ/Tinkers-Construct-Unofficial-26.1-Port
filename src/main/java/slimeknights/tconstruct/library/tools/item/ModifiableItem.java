...
public class ModifiableItem extends TieredItem implements IModifiableDisplay {
  ...
  protected static boolean shouldInteract(@Nullable LivingEntity player, ToolStack toolStack, InteractionHand hand) {
    ...
    if (volatileData.getBooleanOr(NO_INTERACTION, false)) {
      return false;
    }
    ...
    return player == null || !volatileData.getBooleanOr(DEFER_OFFHAND, false) || player.getOffhandItem().isEmpty();
  }
  ...
  public boolean canPerformAction(ItemInstance stack, ItemAbility toolAction) {
    return stack instanceof ItemStack itemStack && itemStack.getCount() == 1
  ...
  public static boolean shouldCauseReequip(ItemStack oldStack, ItemStack newStack, boolean slotChanged) {
    ...
    List<ItemAttributeModifiers.Entry> attributesNew = newStack.getAttributeModifiers().modifiers().stream()
      .filter(entry -> entry.slot().test(EquipmentSlot.MAINHAND)).toList();
    List<ItemAttributeModifiers.Entry> attributesOld = oldStack.getAttributeModifiers().modifiers().stream()
      .filter(entry -> entry.slot().test(EquipmentSlot.MAINHAND)).toList();
