...
public class Util {
  ...
  private static int calcColor(DyeColor color) {
    return ARGB.opaque(color.getTextureDiffuseColor());
  }
  ...
  public static UseOnContext offset(UseOnContext context, BlockPos offset) {
    ...
    return new UseOnContext(context.getLevel(), context.getPlayer(), context.getHand(), context.getItemInHand(), offset(getHitResult(context), offset));
  }

  /** Reconstructs the protected hit result of a use context using its public 26.1 accessors. */
  public static BlockHitResult getHitResult(UseOnContext context) {
