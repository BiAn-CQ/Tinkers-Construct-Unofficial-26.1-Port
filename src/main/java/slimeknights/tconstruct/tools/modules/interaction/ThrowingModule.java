...
public enum ThrowingModule implements ModifierModule, GeneralInteractionModifierHook, UsingToolModifierHook, ToolActionModifierHook {
  INSTANCE;

  private static final List<ModuleHook<?>> DEFAULT_HOOKS = HookProvider.<ThrowingModule>defaultHooks(ModifierHooks.GENERAL_INTERACT, ModifierHooks.TOOL_USING, ModifierHooks.TOOL_ACTION);
  ...
  public ItemUseAnimation getUseAction(IToolStackView tool, ModifierEntry modifier) {
    return ModifierUtil.blockWhileCharging(tool, ItemUseAnimation.TRIDENT);
  }

  @Override
  public boolean canPerformAction(IToolStackView tool, ModifierEntry modifier, ItemAbility toolAction) {
    return toolAction == ItemAbilities.TRIDENT_THROW;
  }
  ...
  public void onStoppedUsing(IToolStackView tool, ModifierEntry modifier, LivingEntity entity, int timeLeft) {
    if (entity instanceof Player player) {
      ...
      if (chargeTime > 10) {
        ...
        level.playSound(null, thrown.getX(), thrown.getY(), thrown.getZ(), SoundEvents.TRIDENT_THROW.value(), SoundSource.PLAYERS, 1, 1);
        if (!player.getAbilities().instabuild) {
