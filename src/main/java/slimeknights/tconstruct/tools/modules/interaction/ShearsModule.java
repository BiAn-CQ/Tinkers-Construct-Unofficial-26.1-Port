...
public record ShearsModule(float flatBonus, float perLevelBonus, float expandedBonus, ModifierCondition<IToolStackView> condition) implements ModifierModule, EntityInteractionModifierHook, ToolActionModifierHook, ConditionalModule<IToolStackView> {
  ...
  public InteractionResult beforeEntityUse(IToolStackView tool, ModifierEntry modifier, Player player, Entity target, InteractionHand hand, InteractionSource source) {
    ...
    int looting = LootingModifierHook.getLooting(tool, context, player.getItemInHand(hand).getEnchantmentLevel(RegistryHelper.getHolder(player.registryAccess(), Enchantments.LOOTING)));
    ...
    if (shearEntity(stack, tool, world, player, target, looting)) {
      ...
      if (!broken) {
        ...
        float expanded = flatBonus + perLevelBonus * (modifier.getEffectiveLevel() - 1) + expandedBonus * tool.getVolatileData().getIntOr(IModifiable.EXPANDED, 0);
        if (expanded > 0) {
