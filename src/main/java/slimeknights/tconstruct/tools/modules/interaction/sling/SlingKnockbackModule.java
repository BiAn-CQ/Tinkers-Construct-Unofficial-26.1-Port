...
public record SlingKnockbackModule(LevelingValue forceMultiplier, float drawtimeMultiplier, float damageMultiplier, IJsonPredicate<LivingEntity> target, ModifierCondition<IToolStackView> condition) implements SlingModule, MeleeHitModifierHook, MeleeDamageModifierHook {
  ...
  public float beforeMeleeHit(IToolStackView tool, ModifierEntry modifier, ToolAttackContext context, float damage, float baseKnockback, float knockback) {
    if (tool.getPersistentData().getBooleanOr(IS_BONKING, false)) {
      return 0;
    }
    return knockback;
  }
  ...
  public float getMeleeDamage(IToolStackView tool, ModifierEntry modifier, ToolAttackContext context, float baseDamage, float damage) {
    if (damageMultiplier != 1 && tool.getPersistentData().getBooleanOr(IS_BONKING, false)) {
          ...
          if (this.target.matches(target)) {
            ...
            if (mop.getType() != HitResult.Type.BLOCK || targetDist < mop.getBlockPos().distToCenterSqr(start)) {
              ...
              if (didBonk) {
                ...
                player.getCooldowns().addCooldown(net.minecraft.core.registries.BuiltInRegistries.ITEM.getKey(tool.getItem()), 3);
