    ...
    if (!tool.isBroken() && tool.getHook(ToolHooks.INTERACTION).canInteract(tool, modifier.getId(), source)) {
      ...
      if (!fluid.isEmpty()) {
        ...
        if (recipe.hasEntityEffects()) {
          ...
          player.getCooldowns().addCooldown(net.minecraft.core.registries.BuiltInRegistries.ITEM.getKey(tool.getItem()), (int)(20 / ConditionalStatModifierHook.getModifiedStat(tool, player, ToolStats.DRAW_SPEED)));
          ...
          if (!world.isClientSide()) {
            ...
            if (!fluid.isEmpty()) {
              float range = 1 + tool.getVolatileData().getIntOr(IModifiable.EXPANDED, 0);
