...
public class ThrownShuriken extends Projectile implements ToolProjectile, ProjectileWithPower, ProjectileWithKnockback, ReusableProjectile {
  ...
  public void tick() {
    ...
    if (hit.getType() == HitResult.Type.BLOCK) {
      ...
      if (state.is(Blocks.NETHER_PORTAL) && state.getBlock() instanceof Portal portal) {
        this.setAsInsidePortal(portal, pos);
        teleported = true;
      } else if (state.is(Blocks.END_GATEWAY) && state.getBlock() instanceof Portal portal) {
        if (state.is(Blocks.END_GATEWAY) && !this.level().isClientSide()
            && this.canUsePortal(false) && this.level().getBlockEntity(pos) instanceof TheEndGatewayBlockEntity gateway && !gateway.isCoolingDown()) {
          this.setAsInsidePortal(portal, pos);
  ...
  protected void addAdditionalSaveData(ValueOutput tag) {
    ...
    tag.store(KEY_STACK, TinkerValueCodecs.ITEM_STACK, this.stack);
    tag.putFloat(KEY_WATER_INERTIA, this.entityData.get(WATER_INERTIA));
    if (!this.tasks.isEmpty()) {
      tag.store(KEY_TASKS, TinkerValueCodecs.COMPOUND_LIST, java.util.stream.StreamSupport.stream(this.tasks.serialize().spliterator(), false)
  ...
  protected void readAdditionalSaveData(ValueInput tag) {
    ...
    tag.read(KEY_STACK, TinkerValueCodecs.ITEM_STACK).ifPresent(this::setStack);
    this.entityData.set(WATER_INERTIA, tag.getFloatOr(KEY_WATER_INERTIA, 0.0f));
    tag.read(KEY_TASKS, TinkerValueCodecs.COMPOUND_LIST).ifPresent(list -> {
