...
public class CombatFishingHook extends FishingHook implements ProjectileWithKnockback, ProjectileWithPower {
  ...
  /** Local lifetime while stuck in a grapple wall; vanilla keeps its lifetime private. */
  ...
  public CombatFishingHook(Player player, Level level, int luck, int lure, float velocity, float inaccuracy) {
    ...
    deltaMovement = deltaMovement.multiply(
      velocity / length + this.getRandom().triangle(0.5, maxRandom),
      velocity / length + this.getRandom().triangle(0.5, maxRandom),
      velocity / length + this.getRandom().triangle(0.5, maxRandom));
  ...
  private void pullGrapple(Entity owner) {
    ...
    if (isDrill() && owner instanceof Player player) {
      ItemStack fishingTool = player.getMainHandItem().canPerformAction(ItemAbilities.FISHING_ROD_CAST)
        ? player.getMainHandItem() : player.getOffhandItem();
      player.startAutoSpinAttack(20, 0.0F, fishingTool);
    }
  ...
  public void tick() {
    ...
    int oldLife = this.wallLife;
    super.tick();
    if (this.wallState != null && !level().isClientSide()) {
      this.wallLife = oldLife + 1;
  ...
  protected void readAdditionalSaveData(ValueInput input) {
    ...
    setMaterial(Objects.requireNonNullElse(MaterialVariantId.tryParse(input.getStringOr(TAG_MATERIAL, "")), IMaterial.UNKNOWN_ID));
  }
}
