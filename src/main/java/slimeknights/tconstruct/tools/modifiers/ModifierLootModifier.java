...
public class ModifierLootModifier extends LootModifier {
  ...
  protected ObjectArrayList<ItemStack> doApply(ObjectArrayList<ItemStack> generatedLoot, LootContext context) {
    // tool is for harvest
    net.minecraft.world.item.ItemInstance toolInstance = context.getOptionalParameter(LootContextParams.TOOL);
    ItemStack stack = toolInstance instanceof ItemStack itemStack ? itemStack : null;
    ...
    if (stack != null && stack.is(TinkerTags.Items.LOOT_CAPABLE_TOOL)) {
      ...
      if (!toolStack.isBroken()) {
        for (ModifierEntry entry : toolStack.getModifierList()) {
          entry.getHook(ModifierHooks.PROCESS_LOOT).processLoot(toolStack, entry, generatedLoot, context);
        }
      }
    }
    return generatedLoot;
