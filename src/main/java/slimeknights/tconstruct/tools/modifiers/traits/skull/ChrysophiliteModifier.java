...
public class ChrysophiliteModifier extends NoLevelsModifier implements EquipmentChangeModifierHook {
    ...
    if (context.getChangedSlot() == EquipmentSlot.HEAD) {
      context.getTinkerData().ifPresent(data -> {
        ...
        if (gold == null) {
          ...
          gold.setGold(EquipmentSlot.HEAD, tool.getVolatileData().getBooleanOr(ModifiableArmorItem.PIGLIN_NEUTRAL, false));
        }
      });
    }
  }
  ...
  public static boolean hasGold(EquipmentChangeContext context, EquipmentSlot slotType) {
    ...
    if (tool != null) {
      return tool.getVolatileData().getBooleanOr(ModifiableArmorItem.PIGLIN_NEUTRAL, false);
    } else {
  ...
  public static int getTotalGold(@Nullable Entity entity) {
    return Optional.ofNullable(entity)
                   .flatMap(TinkerDataCapability::getDataOptional)
