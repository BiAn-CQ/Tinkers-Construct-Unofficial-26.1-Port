...
public class ModifierCrystalItem extends Item {
  ...
  @Override
  public void appendHoverText(ItemStack stack, Item.TooltipContext context, TooltipDisplay display, Consumer<Component> tooltip, TooltipFlag advanced) {
    List<Component> lines = new java.util.ArrayList<>();
    ModifierId id = getModifier(stack);
    if (id != null) {
      if (ModifierManager.INSTANCE.contains(id)) {
        lines.addAll(ModifierManager.INSTANCE.get(id).getDescriptionList());
      }
      lines.add(TOOLTIP_APPLY);
      if (advanced.isAdvanced()) {
        lines.add((Component.translatable(MODIFIER_KEY, id.toString())).withStyle(ChatFormatting.DARK_GRAY));
      }
    } else {
  ...
  public boolean overrideStackedOnOther(ItemStack stack, Slot slot, ClickAction action, Player player) {
    ...
    if (action == ClickAction.SECONDARY && slot.allowModification(player) && CreativeSlotItem.canApply(player)) {
      ...
      if (modifier != null && !toolItem.isEmpty() && toolItem.is(TinkerTags.Items.MODIFIABLE)) {
        if (CreativeSlotItem.shouldHandleStackInteraction(player)) {
  ...
  public boolean overrideOtherStackedOnMe(ItemStack stack, ItemStack toolItem, Slot slot, ClickAction action, Player player, SlotAccess access) {
    ...
    if (action == ClickAction.SECONDARY && slot.allowModification(player) && !toolItem.isEmpty() && toolItem.is(TinkerTags.Items.MODIFIABLE) && (player.isCreative() || MantleCommand.hasPermission(player, MantleCommand.PERMISSION_GAME_COMMANDS))) {
      // NBT must be valid
      ModifierId modifier = getModifier(stack);
      if (modifier != null) {
        if (CreativeSlotItem.shouldHandleStackInteraction(player)) {
  ...
  public static ItemStack withModifier(ModifierId modifier, int count) {
    ...
    ItemStackDataUtil.updateTag(stack, tag -> tag.putString(TAG_MODIFIER, modifier.toString()));
    return stack;
  }
