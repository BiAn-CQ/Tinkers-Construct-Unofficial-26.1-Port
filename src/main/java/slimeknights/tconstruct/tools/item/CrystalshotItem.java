...
public class CrystalshotItem extends ArrowItem {
  ...
  public AbstractArrow createArrow(Level pLevel, ItemStack pStack, LivingEntity pShooter, ItemStack weapon) {
    ...
    CompoundTag tag = ItemStackDataUtil.getTag(pStack);
  ...
  public boolean isInfinite(ItemStack stack, ItemStack bow, LivingEntity shooter) {
    return bow.getEnchantmentLevel(RegistryHelper.getHolder(shooter.registryAccess(), Enchantments.INFINITY)) > 0;
  }
  ...
  public static ItemStack withVariant(String variant, int size) {
    ...
    ItemStackDataUtil.updateTag(stack, tag -> tag.putString(TAG_VARIANT, variant));
    return stack;
  }
  ...
  public static class CrystalshotEntity extends AbstractArrow {
    ...
    public CrystalshotEntity(Level level, LivingEntity shooter, ItemStack pickupItem, @Nullable ItemStack weapon) {
      super(TinkerTools.crystalshotEntity.get(), shooter, level, pickupItem.copyWithCount(1), weapon);
      setSoundEvent(Sounds.CRYSTALSHOT.getSound());
    }
