...
public class ToolEvents {
    ...
    if (block instanceof BeehiveBlock beehive) {
      ...
      if (level >= 5) {
        ...
        if (!CampfireBlock.isSmokeyPos(world, pos)) {
          // 26.1 no longer exposes the two intermediate bee helpers; the public
          // release call below still performs the vanilla emergency behavior.
  ...
  static void livingAttack(LivingIncomingDamageEvent event) {
    ...
    if (entity.level() instanceof ServerLevel server && entity.isInvulnerableTo(server, source)) {
      return;
    }
  ...
  static void livingHurt(LivingIncomingDamageEvent event) {
    ...
    if (source.is(TinkerTags.DamageTypes.FIRE_PROTECTION)) {
      int level = TinkerEffect.getLevel(entity, TinkerEffects.conductive.getDelegate());
      if (level > 0) {
    ...
    if (source.is(TinkerTags.DamageTypes.MAGIC_PROTECTION)) {
      int level = TinkerEffect.getLevel(entity, TinkerEffects.venom.getDelegate());
      if (level > 0) {
        originalDamage *= Math.pow(2, level);
      }
    }
    ...
    if (vanillaModifier != modifierValue || (cap > 20 && vanillaModifier > 20) || (cap < 20 && vanillaModifier > cap)) {
      ...
      float finalDamage = ArmorUtil.getDamageForEvent(entity, event.getSource(), originalDamage, armor, toughness, vanillaModifier, modifierValue, cap);
      ...
      if (!source.is(DamageTypeTags.BYPASSES_ARMOR)) {
        ...
        if (damageMissed > 0 && entity instanceof Player) {
          for (EquipmentSlot slotType : ModifiableArmorMaterial.ARMOR_SLOTS) {
            ...
            boolean damageableBySource = armorStack.canBeHurtBy(source);
