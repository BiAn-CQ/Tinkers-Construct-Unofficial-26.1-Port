...
public class ModifierRemovalRecipe extends AbstractWorktableRecipe {
  ...
  private final List<ItemStackTemplate> leftovers;
  ...
  public ModifierRemovalRecipe(Identifier id, String name, SizedIngredient toolRequirement, List<SizedIngredient> inputs, List<?> leftovers, IJsonPredicate<ModifierId> modifierPredicate) {
    ...
    this.leftovers = leftovers.stream().map(stack -> stack instanceof ItemStackTemplate template ? template : ItemStackTemplate.fromNonEmptyStack((ItemStack)stack)).toList();
  ...
  public void updateInputs(LazyToolStack result, ITinkerableContainer.Mutable inv, ModifierEntry selected, boolean isServer) {
    ...
    if (isServer) {
      for (ItemStackTemplate template : leftovers) {
        inv.giveItem(template.create());
      }
    }
  }
  ...
  public List<ItemStack> getInputTools() {
    if (tools == null) {
      tools = sizedTool.getMatchingStacks().stream().map(stack -> {
        ...
        if (stack.getCount() > 1) {
          tool = slimeknights.tconstruct.library.utils.ItemStackDataUtil.copyStackWithSize(tool, stack.getCount());
        }
        return tool;
      }).toList();
    }
