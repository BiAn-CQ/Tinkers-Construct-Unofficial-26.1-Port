...
public class PartBuilderScreen extends BaseTabbedScreen<PartBuilderBlockEntity,PartBuilderContainerMenu> {
  ...
  public PartBuilderScreen(PartBuilderContainerMenu container, Inventory playerInventory, Component title) {
    ...
    this.infoPanelScreen.setLayoutHeight(this.imageHeight);
  ...
  protected void renderBg(GuiGraphicsExtractor graphics, float partialTicks, int mouseX, int mouseY) {
    ...
    // draw scrollbar
    graphics.blit(RenderPipelines.GUI_TEXTURED, BACKGROUND, this.cornerX + SLIDER_LEFT,
      this.cornerY + SLIDER_TOP + (int) (SROLLABLE_AREA * this.sliderProgress),
      canScroll() ? HANDLE_U : HANDLE_U_DISABLE, 0, SLIDER_WIDTH, HANDLE_HEIGHT, 256, 256);
  ...
  private void drawRecipesBackground(GuiGraphicsExtractor graphics, int mouseX, int mouseY, int left, int top) {
    ...
    for (int i = this.recipeIndexOffset; i < max; i++) {
      ...
      graphics.blit(RenderPipelines.GUI_TEXTURED, BACKGROUND, x, y, PATTERN_U, v, PATTERN_SIZE, PATTERN_SIZE, 256, 256);
    }
  }
  ...
  public boolean mouseDragged(MouseButtonEvent event, double dragX, double dragY) {
    ...
    double timeSinceLastClick = dragX;
