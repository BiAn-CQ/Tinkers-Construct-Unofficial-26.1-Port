...
public class SideButtonsWidgetPaged<T extends Button> extends SideButtonsWidget<T> {
    ...
    if (mouseButton == 0) {
            ...
            if (button_num > columns * MAX_ROWS) { // Page button shows
                if (this.previousPageButton.mouseClicked(event, false)) {
                    this.clickedButtonIndex = 1;
                    return true;
                }
                if (this.nextPageButton.mouseClicked(event, false)) {
    ...
    public boolean handleMouseReleased(double mouseX, double mouseY, int state) {
        MouseButtonEvent event = new MouseButtonEvent(mouseX, mouseY, new MouseButtonInfo(state, 0));
        if (this.clickedButtonIndex == 1) {
            this.previousPageButton.mouseReleased(event);
            this.clickedButtonIndex = 0;
            return true;
        }
        if (this.clickedButtonIndex == 2) {
            this.nextPageButton.mouseReleased(event);
            this.clickedButtonIndex = 0;
            return true;
        }
