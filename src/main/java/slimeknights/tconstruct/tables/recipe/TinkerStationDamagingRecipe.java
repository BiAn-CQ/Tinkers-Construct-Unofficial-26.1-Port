...
public class TinkerStationDamagingRecipe implements ITinkerStationRecipe, IDisplayToolModification {
  ...
  public TinkerStationDamagingRecipe(Identifier id, Ingredient ingredient, int damageAmount) {
    this.id = id;
  ...
  @Override
  public RecipeSerializer<TinkerStationDamagingRecipe> getSerializer() {
  ...
  public List<ItemStack> getDisplayItems(int slot) {
    return slot == 0 ? List.of(slimeknights.tconstruct.library.recipe.TinkerIngredients.getItems(ingredient)) : List.of();
  }
      ...
      toolWithoutModifier = RegistryHelper.getTagValueStream(BuiltInRegistries.ITEM, TinkerTags.Items.DURABILITY)
        .map(item -> {
          if (item instanceof IModifiableDisplay modifiable) {
            ...
            slimeknights.tconstruct.library.utils.ItemStackDataUtil.updateTag(stack, tag -> tag.put(ToolStack.TAG_STATS, stats));
            return stack;
          }
  ...
  public List<ItemStack> getToolWithModifier() {
    if (toolWithModifier == null) {
      toolWithModifier = getToolWithoutModifier().stream()
        .map(stack -> {
          stack = stack.copy();
          slimeknights.tconstruct.library.utils.ItemStackDataUtil.updateTag(stack, tag -> tag.putInt(ToolStack.TAG_DAMAGE, damageAmount));
          return stack;
        }).toList();
    }
