...
public class ItemTagProvider extends IntrinsicHolderTagsProvider<Item> {
  ...
  /** Legacy common spear tag, no longer exposed as a NeoForge constant. */
  private static final TagKey<Item> TOOLS_SPEAR = ItemTags.create(commonResource("tools/spear"));
  private final Function<Identifier,TagAppender<Item,Item>> MAKE_TAG = id -> tag(ItemTags.create(id));
  private final CompletableFuture<TagLookup<Block>> blockTags;

  public ItemTagProvider(PackOutput output, CompletableFuture<Provider> lookupProvider, CompletableFuture<TagLookup<Block>> blockTags) {
    super(output, Registries.ITEM, lookupProvider,
      item -> BuiltInRegistries.ITEM.getResourceKey(item).orElseThrow(), TConstruct.MOD_ID);
  ...
  private void addCommon() {
    // Keep the casting output tag populated on 1.21.1.
    Identifier amethystStorageBlock = commonResource("storage_blocks/amethyst");
    this.copy(TagKey.create(Registries.BLOCK, amethystStorageBlock), TagKey.create(Registries.ITEM, amethystStorageBlock));
    ...
    this.tag(TinkerTags.Items.TRIM_TEMPLATES).add(
    ...
    // glass
    copy(TinkerTags.Blocks.GLASS_BLOCKS_SILICA, TinkerTags.Items.GLASS_BLOCKS_SILICA);
    ...
    for (DyeColor color : DyeColor.values()) {
      Identifier name = commonResource("dyed/" + color.getSerializedName());
  ...
  private void addTools() {
    ...
    addToolTags(TinkerTools.javelin,    MULTIPART_TOOL, DURABILITY, MELEE_PRIMARY, RANGED,      INTERACTABLE_RIGHT, BROAD_RANGED, BONUS_SLOTS, TOOLS_SPEAR);
    ...
    this.tag(TinkerTags.Items.STORAGE_BLOCKS_QUARTZ).add(Items.QUARTZ_BLOCK);
  ...
  /** Copies block tag entries to the item registry, replacing the removed ItemTagsProvider helper. */
  protected void copy(TagKey<Block> blockTag, TagKey<Item> itemTag) {
    TagBuilder source = blockTags.join().apply(blockTag)
      .orElseThrow(() -> new IllegalStateException("Missing block tag " + blockTag.location()));
    TagBuilder target = getOrCreateRawBuilder(itemTag).setReplace(source.shouldReplace());
    source.build().forEach(target::add);
    source.getRemoveEntries().forEach(target::remove);
  }
