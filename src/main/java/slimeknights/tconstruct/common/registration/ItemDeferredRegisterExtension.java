...
public class ItemDeferredRegisterExtension extends ItemDeferredRegister {
  ...
  public CastItemObject registerCast(String name, Item.Properties props) {
    // The same properties object is used for all three cast variants, but in
    // 26.1 the Item constructor derives the default ITEM_MODEL component from
    // the current item id. Bind it inside the supplier so each variant gets
    // its own model id instead of inheriting an unbound property object.
    return registerCast(name, () -> new Item(ItemDeferredRegister.bindItemProperties(props)));
  }
  ...
  public CastItemObject registerCast(ItemObject<? extends IMaterialItem> item, Item.Properties props) {
    return registerCast(item.getId().getPath(), () -> new PartCastItem(ItemDeferredRegister.bindItemProperties(props), item));
  }
}
