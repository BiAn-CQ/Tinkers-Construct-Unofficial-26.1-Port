...
public class EntityMeltingModule {
  ...
  private EntityMeltingRecipe findRecipe(EntityType<?> type) {
    ...
    EntityMeltingRecipe recipe = EntityMeltingRecipeCache.findRecipe(slimeknights.tconstruct.library.utils.TinkerRecipeHelper.getRecipeManager(getLevel()), type);
    if (recipe != null) {
      lastRecipe = recipe;
    }
    return recipe;
  ...
  public boolean interactWithEntities() {
    ...
    for (Entity entity : getLevel().getEntitiesOfClass(Entity.class, boundingBox)) {
      ...
      // entities in the melting blacklist cannot be melted
      else if (canMelt != Boolean.FALSE && !type.builtInRegistryHolder().is(EntityTypes.MELTING_BLACKLIST) && entity instanceof LivingEntity && canMeltEntity((LivingEntity)entity)) {
