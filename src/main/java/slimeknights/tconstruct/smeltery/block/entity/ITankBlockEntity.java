...
public interface ITankBlockEntity extends IFluidTankUpdater, FluidUpdatePacket.IFluidPacketReceiver {
  ...
  /**
   * Invalidates the block-entity model data and, on the client, notifies the
   * block renderer to query it again.  {@code requestModelDataUpdate()} alone
   * only invalidates the cached data; it does not necessarily enqueue a model
   * rebuild when the block state itself did not change.
   */
  default void refreshFluidModelData() {
    BlockEntity te = getTE();
    ...
    if (level != null && level.isClientSide()) {
      ...
      level.sendBlockUpdated(pos, state, state, 48);
    }
  }
