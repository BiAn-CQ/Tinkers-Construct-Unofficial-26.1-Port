...
public record MaterialTagSource(MaterialManager manager) implements TagSource<IMaterial> {
  ...
  public List<Identifier> keysInTag(TagKey<IMaterial> tag) {
    ...
    return entries.stream().map(IMaterial::getIdentifier).map(MaterialId::location).collect(Collectors.toList());
  }
  ...
  public Stream<Identifier> valueKeys() {
    return manager.getAllMaterials().stream().map(IMaterial::getIdentifier).map(MaterialId::location);
  }
}
