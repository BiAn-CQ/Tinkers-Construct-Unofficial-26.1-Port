...
public class AntigravityEffect extends TinkerEffect {
  ...
  public AntigravityEffect() {
    ...
    this.addAttributeModifier(Attributes.GRAVITY, TConstruct.getResource("5bd6b8c8-8de9-4357-a74e-afb2a8f00c20"), -2, Operation.ADD_MULTIPLIED_TOTAL);
  ...
  public boolean applyEffectTick(ServerLevel level, LivingEntity living, int amplifier) {
    ...
    if (living.getAttributeValue(Attributes.GRAVITY) < 0) {
      if (!living.level().isClientSide()) {
        ...
        if (living.getY() > level.getMaxY() + 5780) {
      ...
      if (living.onClimbable()) {
        ...
        if (living.horizontalCollision || living.isJumping()) {
          y = -0.2;
        }
  ...
  private void onEffectRemoved(MobEffectEvent.Remove event) {
    if (event.getEffect().value() == this) {
      LAST_VELOCITY.remove(event.getEntity().getId());
    }
  }

  private void onEffectExpired(MobEffectEvent.Expired event) {
    if (event.getEffectInstance() != null && event.getEffectInstance().getEffect().value() == this) {
      LAST_VELOCITY.remove(event.getEntity().getId());
    }
  }
