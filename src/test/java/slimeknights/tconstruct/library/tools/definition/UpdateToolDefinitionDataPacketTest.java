...
class UpdateToolDefinitionDataPacketTest extends BaseMcTest {
  private static final Identifier EMPTY_ID = Identifier.fromNamespaceAndPath("test", "empty");
  private static final Identifier FILLED_ID = Identifier.fromNamespaceAndPath("test", "filled");
  ...
  void testGenericEncodeDecode() {
    ...
    ToolDefinitionData filled = ToolDefinitionDataBuilder
      ...
      .module(ToolActionsModule.of(TinkerToolActions.AXE_DIG, ItemAbilities.SHOVEL_FLATTEN))
      // behavior
    ...
    // parse results
    Map<Identifier,ToolDefinitionData> parsedMap = decoded.getDataMap();
    ...
    assertThat(parsed.getHook(ToolHooks.TOOL_ACTION).canPerformAction(tool, TinkerToolActions.AXE_DIG)).isTrue();
