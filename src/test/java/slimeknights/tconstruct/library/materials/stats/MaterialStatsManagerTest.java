...
import slimeknights.tconstruct.test.MergingJsonTestAdapter;
...
class MaterialStatsManagerTest extends BaseMcTest {
  ...
  private final TestMaterialStatsManager materialStatsManager = new TestMaterialStatsManager();
  private final MergingJsonFileLoader<Map<Identifier,JsonObject>> fileLoader = new MergingJsonFileLoader<>(materialStatsManager);

  private static class TestMaterialStatsManager extends MaterialStatsManager implements MergingJsonTestAdapter<Map<Identifier,JsonObject>> {
    TestMaterialStatsManager() {
      super(() -> {});
    }

    @Override
    public Gson gsonForTest() {
      return gson;
    }

    @Override
    public String folderForTest() {
      return folder;
    }

    @Override
    public Function<Identifier,Map<Identifier,JsonObject>> builderConstructorForTest() {
      return builderConstructor;
    }

    @Override
    public void parseForTest(Map<Identifier,JsonObject> builder, Identifier id, JsonElement element) {
      parse(builder, id, element);
    }

    @Override
    public void finishLoadForTest(Map<Identifier,Map<Identifier,JsonObject>> map, ResourceManager manager) {
  ...
  void loadMissingFile_ignored() {
    ...
    fileLoader.loadAndParseFiles(null, new net.minecraft.resources.Identifier[0]);
