...
class UpdateMaterialStatsPacketTest extends BaseMcTest {
  ...
  private static final NamedComponentRegistry<MaterialStatType<?>> LOADER = new NamedComponentRegistry<>("Unknown stat type");
  static {
    register(MaterialStatsFixture.COMPLEX_TYPE);
    register(HeadMaterialStats.TYPE);
    register(HandleMaterialStats.TYPE);
    register(StatlessMaterialStats.BINDING.getType());
  }

  private static void register(MaterialStatType<?> type) {
    LOADER.register(type.getId().location(), type);
  }
